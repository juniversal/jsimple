...
namespace jsimple.util {
    ...
    public class TextualPath {
        ...
        /// </summary>
        /// <param name="prefixPath"> path that potentially is at beginning of this path </param>
        /// <returns> true if this the beginning elements of this path match prefixPath or if the two paths are the same </returns>
        public virtual bool StartsWith(TextualPath prefixPath) {
            int prefixLength = prefixPath.Length;

            if (prefixLength > pathElements.Count)
                return false;

            for (int i = 0; i < prefixLength; i++) {
                string pathElement = pathElements[i];
                string prefixPathElement = prefixPath.pathElements[i];
